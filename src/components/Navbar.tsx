...
import { Play, RotateCcw, Settings, BookOpen, Cpu, Download, FilePlus, CheckCircle2, Globe, Sparkles } from 'lucide-react';
  ...
  return (
    <header className="bg-slate-900 border-b border-slate-800 text-white px-4 py-2.5 flex items-center justify-between select-none">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="hidden md:flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full text-[11px] bg-slate-800/80 border border-slate-700/60 text-slate-300">
          {engineMode === 'native' ? (
            <>
              <CheckCircle2 className="w-3 h-3 text-emerald-400" />
              <span className="text-emerald-300 font-medium">GCC Nativo (Servidor)</span>
            </>
          ) : (
            <>
              <Globe className="w-3 h-3 text-sky-400" />
              <span className="text-sky-300 font-medium">GCC Cloud (Online)</span>
            </>
          )}
        </div>
      </div>
